...
likely_factors <- function(df) {
  ...
  names(df)[vapply(df, is_codey, logical(1))]
}

likely_factors(chapter_7_table_15)   # e.g., Sex, Education
likely_factors(chapter_16_table_1)   # e.g., Gender, Trainee, Condition
```

This is a heuristic, not a guarantee (a genuine count variable can look
"codey," and a categorical variable can have many levels). Always
confirm a variable's role, and the meaning of its codes, from its help
page (for example `?chapter_7_table_15`) and from the book before
